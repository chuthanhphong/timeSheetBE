...
public class TimeLineController {
    ...
    @GetMapping(ConstantsTimeLine.EXPORT_EXCEL)
    public ResponseEntity<?> getListTimeLineForExcelExport(@ModelAttribute ListTineLineRequest request) {
        if (request.getProjectId() == null) {
            return ResponseEntity.badRequest().body(ConstantsTimeLine.NO_PROJECT);
        } else if (request.getStartDate() == null || request.getEndDate() == null) {
            return ResponseEntity.badRequest().body(ConstantsTimeLine.NO_DATE);
        } else {
            return ResponseEntity.ok(timeLineService.getListTimeLine(request, Pageable.unpaged()));
        }
    }
    ...
    public ResponseEntity<?> getNoteTimeLineDetail(@RequestParam String timeSheetId) {
