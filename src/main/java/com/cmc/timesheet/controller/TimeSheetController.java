...
public class TimeSheetController {
    ...
    public ResponseEntity<?> getTimeSheetDataForExcel(@RequestBody TimeSheetFilterRequest timeSheetFilterRequest) {
        List<TimeSheetResponse> timeSheetResponses = timeSheetService.getTimeSheetDataForExcel(timeSheetFilterRequest);

        if (!ObjectUtils.isNullorEmpty(timeSheetResponses)) {
            return ResponseEntity.ok(timeSheetResponses);
        }
            return new ResponseEntity<>(HttpStatus.BAD_REQUEST);

    }

    @GetMapping(value="/excel-view-detail")
    public ResponseEntity<?> getTimeSheetDataForExcel(@ModelAttribute TimeSheetViewDetailRequest request) {
        List<TimeSheetViewDetailResponse> timeSheetResponses = timeSheetService.getListTimeSheetExcelByEmployeeId(request);
